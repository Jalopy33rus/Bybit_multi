...
# -*- coding: utf-8 -*-
"""
ULTIMATE TRADING BOT v7.0 - BYBIT FUTURES WITH HYBRID TREND-CORRECTION STRATEGY
Полностью исправленная версия с ультра-консервативным подходом
"""
...
# ====== НАСТРОЙКИ С ГИБРИДНОЙ СТРАТЕГИЕЙ ======
TRADING_MODES = {
    "ULTRA_CONSERVATIVE": {
        ...
        "min_risk_reward": 3.0,  # Минимальное соотношение риск/вознаграждение
        
        # Фильтры тренда:
        "min_trend_strength": 35,  # Только сильные тренды
        "max_trend_age": 25,
        "require_trend_alignment": True,
        "require_trend_confirmation": True,
        
        # RSI фильтры:
        "rsi_range_long": (30, 65),   # Уже диапазон
        "rsi_range_short": (35, 70),  # Уже диапазон
        
        # Фильтры объема
        "volume_multiplier": 2.0,  # Увеличили требование
        "min_volume_score": 20,
        
        # Фильтры волатильности
        "max_atr_percentage": 0.08,   # 8%
        "min_atr_percentage": 0.02,   # 2%
        "bb_width_min": 0.015,        # Минимальная ширина BB
        
        # Общий фильтр
        "min_score": 110,  # Повысили требование
        ...
        "trailing_stop_update_frequency": 0.0015,
    ...
    "CONSERVATIVE": {
        ...
        "rsi_range_long": (28, 70),
        "rsi_range_short": (30, 72),
        
        "volume_multiplier": 1.8,
        "min_volume_score": 15,
        
        "max_atr_percentage": 0.09,
        "min_atr_percentage": 0.018,
        "bb_width_min": 0.012,
...
LOCK_FILE = "/tmp/ultimate_trading_bot_v7.lock"
DB_FILE = "trades_ultimate_futures_v7.db"

# Глобальные переменные
CURRENT_MODE = "AGGRESSIVE"  # По умолчанию самый безопасный режим
BOT_RUNNING = True
exchange = None
bot = None
updater = None
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('ultimate_bot_futures_v7.log', encoding='utf-8'),
...
def get_current_settings():
    ...
    return TRADING_MODES.get(CURRENT_MODE, TRADING_MODES["ULTRA_CONSERVATIVE"])
        ...
        if settings.get('require_trend_alignment', True) and timeframe in ["1h", "30m"]:
            try:
                ...
                if df_higher is not None and len(df_higher) > 20:
                    ...
                    price_change_higher = (df_higher['close'].iloc[-1] - df_higher['close'].iloc[-5]) / df_higher['close'].iloc[-5]
                    
                    if direction in ["BULLISH", "WEAK_BULLISH"] and price_change_higher < -0.002:
                        confirmed = False
                    elif direction in ["BEARISH", "WEAK_BEARISH"] and price_change_higher > 0.002:
                        confirmed = False
...
def calculate_position_size(symbol: str, signal_score: int, available_usdt: float):
    ...
    try:
        ...
        # Учет волатильности символа
        volatility_factor = SYMBOL_CATEGORIES.get(symbol, {}).get("volatility", "MEDIUM")
        if volatility_factor == "HIGH":
            bonus_pct *= 0.7  # Уменьшаем для высоковолатильных
        elif volatility_factor == "LOW":
            bonus_pct *= 1.2  # Увеличиваем для низковолатильных
        
        total_pct = min(base_pct + bonus_pct, 0.02)  # Макс 2%
...
def commission_filter(symbol: str, entry_price: float, take_profit: float, 
    ...
    """Фильтр: прибыль должна покрывать комиссии + 1%"""
    try:
        ...
        # Фильтр: прибыль > комиссии + 1%
        passes = potential_profit_pct > (total_fee_pct + 1.0)
        
        if not passes:
            logger.info(f"⏹️ Commission filter failed for {symbol}: "
                       f"Profit {potential_profit_pct:.2f}% < Fees+1% {total_fee_pct+1.0:.2f}%")
...
def validate_risk_reward(entry_price: float, stop_loss: float, take_profit: float, position_type: str):
    """Проверка соотношения риск/вознаграждение"""
    try:
        if position_type == 'LONG':
...
# ====== УЛУЧШЕННЫЙ АНАЛИЗ СИМВОЛОВ С ГИБРИДНОЙ СТРАТЕГИЕЙ ======
def analyze_symbol_with_filters(symbol: str) -> Optional[Dict]:
    """Анализ символа со всеми фильтрами и гибридной стратегией"""
    try:
        settings = get_current_settings()
        ...
        if check_weekly_limit():
            logger.info(f"⏹️ Weekly trade limit reached")
            return None
        ...
        position_type = "LONG" if trend_analysis["direction"] in ["BULLISH", "WEAK_BULLISH"] else "SHORT"
        ...
        if position_type == "LONG":
            ...
            # Проверка 1: Цена около нижней BB
            ...
            if price_to_ema20 < 0.01 or price_to_ema50 < 0.015:
                price_at_key_level = True
                correction_depth = min(price_to_ema20, price_to_ema50)
            
            # Проверка согласованности MACD
        ...
        if bb_width < settings.get('bb_width_min', 0.01):
            logger.info(f"⏹️ {symbol} filtered: low volatility (BB width {bb_width:.3%} < {settings.get('bb_width_min', 0.01):.3%})")
            return None
        ...
        # Объем (макс 20)
        volume_score = min(volume_ratio * 10, 20) if volume_ratio >= settings['volume_multiplier'] else 0
        score += volume_score
        if volume_score > 0:
        ...
        # Коррекция к ключевому уровню (макс 25)
        if price_at_key_level:
            correction_score = min(correction_depth * 100, 25)
...
def open_position(signal: Dict):
    ...
    try:
        ...
        # Расчет SL/TP с адаптацией под волатильность
        ...
        # Установка SL/TP в зависимости от направления
        if position_type == 'LONG':
        ...
        # Проверка risk/reward
        rr_passes, rr_ratio = validate_risk_reward(current_price, stop_loss, take_profit_price, position_type)
        if not rr_passes:
            logger.info(f"⏹️ Risk/Reward ratio too low for {symbol}: {rr_ratio:.2f}")
            return False
        ...
        """, (
            ...
            signal.get('atr', 0), signal.get('trend_strength', 0), signal_score, rr_ratio
        ))
        ...
        logger.info(f"   Risk/Reward: {rr_ratio:.2f}, Score: {signal_score}")
...
<b>Улучшения v7.0:</b>
• ✅ Ультра-консервативный режим
• ✅ Гибридная стратегия (тренд + коррекции)
• ✅ Фильтр комиссий
• ✅ Адаптивный размер позиции
• ✅ Проверка Risk/Reward (мин 1:3)
• ✅ Исправлен анализ тренда (без рекурсии)
...
def cmd_change_mode(update, context):
    ...
    try:
        ...
        update.message.reply_text(
            f"✅ <b>Режим изменен на: {settings['name']}</b>\n\n"
...
Требуемая прибыль: > {entry_fee + exit_fee + 1.0:.3f}%
"""
        update.message.reply_text(msg, parse_mode=ParseMode.HTML)
...
def main_trading_loop():
    ...
    logger.info("🤖 Starting ULTIMATE TRADING BOT v7.0...")
    ...
    safe_send(
        f"{mode_text} <b>ULTIMATE BOT v7.0 STARTED</b>\n"
        f"Баланс: {balance:.2f} USDT\n"
        f"Режим: {settings['name']}\n" 
        f"Плечо: {settings['leverage']}x\n"
        f"Статус: 🟢 АКТИВЕН\n\n"
        f"<b>Стратегия:</b> Гибридная тренд-коррекционная\n"
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"\n{'='*60}")
        print(f"✅ ULTIMATE TRADING BOT v7.0 STARTED!")
        print(f"{'='*60}")
        ...
        print(f"3. Начните с режима ULTRA_CONSERVATIVE")
        print(f"{'='*60}\n")
